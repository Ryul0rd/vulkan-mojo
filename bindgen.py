...
def emit_wrapper_type(wrapper_type: VkWrapperType) -> str:
    ...
    mojo_type = emit_mojo_type(wrapper_type.underlying_type, use_external_origin=True)
    return (
...
def emit_flags(files: Dict[str, str], flags: List[VkFlags | VkTypeAlias]):
    ...
    for flag in flags:
        ...
        parts.append((
            ...
            f"\n"
            f"    fn __or__(self, other: {flags_name}) -> Self:\n"
            f"        return Self(value = self.value() | other.value())\n"
            f"\n"
            f"    fn __ror__(self, other: {flags_name}) -> Self:\n"
            f"        return Self(value = self.value() | other.value())\n"
            f"\n"
            f"    fn __and__(self, other: {flags_name}) -> Self:\n"
            f"        return Self(value = self.value() & other.value())\n"
            f"\n"
            f"    fn __rand__(self, other: {flags_name}) -> Self:\n"
            f"        return Self(value = self.value() & other.value())\n"
            f"\n"
            f"    fn __contains__(self, bit: {flag_bits_name}) -> Bool:\n"
            f"        return Bool(self.value() & bit.value())\n"
            f"\n"
            f"    fn is_subset(self, other: {flags_name}) -> Bool:\n"
            f"        return self & other == self\n"
            f"\n"
            f"    fn is_superset(self, other: {flags_name}) -> Bool:\n"
        ...
        for member in struct.members:
            ...
            if member.bit_width is None:
                ...
                if is_stype:
                    ...
                    string_member_type = emit_mojo_type(member.type, string_origin=origin_param)
...
def emit_command_wrapper(command: VkCommand, version: Optional[VkVersion]=None) -> str:
    ...
    for arg in command.params:
        ...
        if is_required_ptr_to_scalar:
            ...
            sig_arg_strs.append(f"{mojo_arg_name}: {emit_mojo_type(arg.type, no_cstringslice_origin=True)}")
...
def emit_mojo_type(
    ...
    use_external_origin: bool = False, 
    no_cstringslice_origin: bool = False,
    string_origin: Optional[str] = None,
) -> str:
    MUT_ORIGIN = "MutOrigin.external" if use_external_origin else "MutAnyOrigin"
    IMMUT_ORIGIN = "ImmutOrigin.external" if use_external_origin else "ImmutAnyOrigin"
